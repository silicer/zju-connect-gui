...
// Local patch (see vendor/README.md): mingw-w64 defines __forceinline as
...
// specifiers"). MSVC's own __forceinline does combine with it, so only GCC and
// clang get the plain spelling.
...
typedef struct CONNECTION_INFO {
    ...
    // Local patch (see vendor/README.md): the address the UDP relay asks the
    // SOCKS5 server for, which differs from orig_dest_* for hijacked DNS. The
    // reply is matched by this pair and then re-labelled with orig_dest_*, so
    // the client still sees an answer from the resolver it queried.
    UINT32 relay_dest_ip;
    UINT16 relay_dest_port;
    BOOL   is_tracked;
...
// forwards the *original* destination, and the proxy dials destinations it
// cannot reach through the tunnel on the local machine, so without this a
// lookup the local resolver cannot answer never reaches the tunnel.
static UINT32 g_dns_redirect_ip = 0;
static UINT16 g_dns_redirect_port = 0;

static BOOL dns_redirect_applies(UINT16 dest_port)
{
...
// Local patch (see vendor/README.md): DNS redirect helpers.
static void set_connection_relay_dest(UINT16 src_port, UINT32 dest_ip, UINT16 dest_port);
static BOOL get_connection_relay_dest(UINT16 src_port, UINT32 *dest_ip, UINT16 *dest_port);
            ...
            if (addr.Outbound)
            {
                ...
                else
                {
                    ...
                    if (action == RULE_ACTION_PROXY)
                    {
                        ...
                        // Local patch (see vendor/README.md): a DNS query goes to
                        // the configured resolver instead of the one the client
                        // picked, so a name only the tunnel can resolve still
                        // resolves. Replies are matched by relay_dest_* and then
                        // re-labelled with orig_dest_* (see the relay response
                        // path), which keeps the client's view unchanged.
                        if (dns_redirect_applies(dest_port))
                            set_connection_relay_dest(src_port, g_dns_redirect_ip, g_dns_redirect_port);
    ...
    while (running)
    {
        ...
        if (FD_ISSET(udp_relay_socket, &read_fds))
        {
            ...
            if (recv_len > 0)
            {
                ...
                if (get_connection_relay_dest(from_port, &dest_ip, &dest_port))
                {
            ...
            if (recv_len > 0)
            {
                ...
                if (recv_buf[3] == SOCKS5_ATYP_IPV4 && recv_len >= 10)
                {
                    ...
                    for (int b = 0; b < CONNECTION_HASH_SIZE; b++)
                    {
                        ...
                        while (conn != NULL)
                        {
                            if (!conn->is_ipv6 && conn->relay_dest_ip == src_ip && conn->relay_dest_port == src_port)
                            {
...
static void add_connection(UINT16 src_port, UINT32 src_ip, UINT32 dest_ip, UINT16 dest_port, UINT32 proxy_config_id)
{
    ...
    while (existing != NULL) {
        if (existing->src_port == src_port) {
            ...
            existing->relay_dest_ip = dest_ip;
            existing->relay_dest_port = dest_port;
    ...
    conn->relay_dest_ip = dest_ip;
    conn->relay_dest_port = dest_port;
...
static void add_connection_v6(UINT16 src_port, const UINT8 src_ip6[16], const UINT8 dest_ip6[16], UINT16 dest_port, UINT32 proxy_config_id)
{
    ...
    while (existing != NULL) {
        if (existing->src_port == src_port) {
            ...
            existing->orig_dest_port = dest_port;
            // IPv6 flows are not redirected (see vendor/README.md).
            existing->relay_dest_ip = 0;
            existing->relay_dest_port = 0;
            existing->proxy_config_id = proxy_config_id;
            existing->is_tracked = TRUE;
            existing->last_activity = GetTickCount64();
            ReleaseSRWLockExclusive(&lock);
    ...
    conn->src_ip = 0;
    conn->orig_dest_ip = 0;
    conn->relay_dest_ip = 0;
    conn->relay_dest_port = 0;
    conn->is_ipv6 = TRUE;
...
// Local patch (see vendor/README.md): point an already-tracked IPv4 UDP flow at
// the configured DNS server instead of the resolver the client chose.
static void set_connection_relay_dest(UINT16 src_port, UINT32 dest_ip, UINT16 dest_port)
{
    AcquireSRWLockExclusive(&lock);
...
// Local patch (see vendor/README.md): the destination the UDP relay has to ask
// the SOCKS5 server for. Equals the client's destination unless the flow was
...
static BOOL get_connection_relay_dest(UINT16 src_port, UINT32 *dest_ip, UINT16 *dest_port)
{
    ...
    while (conn != NULL)
    {
        if (conn->src_port == src_port && !conn->is_ipv6)
        {
            *dest_ip = conn->relay_dest_ip;
            *dest_port = conn->relay_dest_port;
            found = TRUE;
            break;
        }
        conn = conn->next;
...
// Local patch (see vendor/README.md): send rule-matched UDP DNS queries to this
// resolver instead of the one the process was pointed at (pass NULL to disable).
PROXYBRIDGE_API void ProxyBridge_SetDnsRedirect(const char* dns_server, int dns_port)
{
    ...
    if (dns_server != NULL && dns_port > 0 && dns_port <= 65535)
    {
        ...
        if (inet_pton(AF_INET, dns_server, &addr) == 1 && addr.s_addr != 0)
        {
            ...
            g_dns_redirect_port = (UINT16)dns_port;
        }
    }
