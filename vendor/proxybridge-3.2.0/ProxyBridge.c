...
// Local patch (see vendor/README.md): set by every public entry point, so the
// exit-time destructor below can tell a host that actually drove this
// library apart from a process that merely linked it in.
static bool g_api_touched = false;
...
static uint32_t get_process_id_from_connection(uint32_t src_ip, uint16_t src_port, bool is_udp)
{
    ...
    // Local patch (see vendor/README.md): 64-bit musl inserts an anonymous
    // `int __pad1` member between `msg_iovlen` and `msg_control`, so the
    // positional initializer upstream uses does not compile there.
    // Designated initializers are valid on glibc and musl alike.
    struct msghdr msg = {
        .msg_name = &sa,
...
{
    // Local patch (see vendor/README.md): as a shared library this destructor
    // only ran when a host unloaded it. Linked in statically it runs on every
    // process exit, so stay quiet unless this process actually used the API.
    if (!g_api_touched)
        return;
